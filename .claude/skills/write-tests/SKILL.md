...
3. **Write** — on approval, author the test files in the locations prescribed by the QA plan (e.g. `src/__tests__/`, `e2e/`). Follow conventions from `docs/specs/qa-plan.md` and any `tests.md` rule file. Reference secrets by env-var name only (e.g. `process.env.STRIPE_WEBHOOK_SECRET`) — never embed literal secret-shaped strings. Use seeded or in-memory test data, not production data.
4. **Run green** — execute `npm test` (or `vitest run`) for unit/integration tests and `npm run test:e2e` for Playwright tests; show the output. If any test fails, diagnose the root cause, fix the test or the fixture, and re-run. Repeat until the full suite for this story is green. Under `full`, show each failure and proposed fix before applying; under `lean` or `solo`, fix and re-run, then show the final result.
