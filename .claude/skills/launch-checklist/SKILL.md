...
6. **Security audit gate (BLOCKING)** — verify that `docs/specs/security-audit.md` exists and contains no open High or Critical findings (look for lines matching `High` or `Critical` that are not marked as resolved/closed). If the file is absent or open High/Critical findings remain, mark this item BLOCK and instruct the user to run `/security-audit` and resolve all High/Critical findings before proceeding.
7. **Tests green gate (BLOCKING)** — run `npm test` (Vitest unit/integration suite) and, if a Playwright config exists, `npx playwright test` for the critical-path e2e suite. If either command exits non-zero or no test files are found, mark this item BLOCK and instruct the user to run `/test-setup` and/or `/write-tests` to establish or fix the test suite before launch.
