...
2. **Group by phase** — organize skills into five groups: (1) Navigation (`start`, `help`, `studio-status`, `scope-check`), (2) Product & UX (`validate-idea`, `write-prd`, `map-flows`, `design-ui`), (3) Engineering (`setup-stack`, `design-architecture`, `design-schema`, `build-feature`, `code-review`), (4) Billing (`design-pricing`, `setup-billing`), (5) Infra & Launch (`setup-deploy`, `launch-checklist`).
